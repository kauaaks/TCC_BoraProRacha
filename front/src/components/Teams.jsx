...
export default function Teams() {
  ...
  const uid =
    user?.firebaseUid || user?.uid ? String(user.firebaseUid || user.uid) : null;
  ...
  const loadAll = async (q) => {
    try {
      setLoading(true);
      const url =
        q && q.trim() ? `/teams?q=${encodeURIComponent(q.trim())}` : "/teams";
      ...
      const allList =
        allResp.status === "fulfilled"
          ? allResp.value.teams || allResp.value || []
          : [];
      const mineList =
        mineResp.status === "fulfilled"
          ? mineResp.value.teams || mineResp.value || []
          : [];

      setTeams(Array.isArray(allList) ? allList : []);
      setMyTeams(Array.isArray(mineList) ? mineList : []);

      if (allResp.status === "rejected")
        console.warn("Falha em /teams:", allResp.reason?.message);
      if (mineResp.status === "rejected")
        console.warn(
          "Falha em /teams/meustimes:",
          mineResp.reason?.message,
        );
    } catch (e) {
      ...
      (Array.isArray(arr) ? arr : [])
        .map((x) => {
          ...
          if (x && typeof x === "object")
            return String(x.firebaseUid || x.uid || x._id || x.id || "");
          return "";
        })
        .filter(Boolean);
  ...
  const notMyTeams = useMemo(() => {
    ...
    return allNormalized.filter((t) => {
      ...
      const ownerHit =
        t.created_by && (t.created_by === uid || myCreators.has(t.created_by));
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Veja todos os times cadastrados
          </p>
        </div>
                  ...
                  try {
                    const response = await apiCall("/teams", {
                      ...
                      body: JSON.stringify({
                        ...
                        monthly_fee:
                          parseFloat(newTeam.monthly_fee) || 0,
                        created_by: uid,
                      }),
                    });
                    ...
                    console.error(err);
                    setError(
                      err.message || "Erro ao criar time",
                    );
                  }
                }}
                className="space-y-4"
